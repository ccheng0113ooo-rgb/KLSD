...
d3.json('data-flare.json', function(error, treeData) {
  ...
  function update(source, transition) {
    ...
    node.select('text')
      ...
      .style("font-size", fontSizeBasedOnDepth);
    ...
    nodeExit.select("text.node-label").style("fill-opacity", 0).remove();
  ...
  function fontSizeBasedOnDepth(d) {
    // 根节点深度为 0，所以可以用它来判断
    return d.depth === 0 ? "26px" : (12 - d.depth) + "px";
  }
